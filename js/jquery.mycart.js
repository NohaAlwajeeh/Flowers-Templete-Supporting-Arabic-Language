...
(function ($) {
  ...
  var OptionManager = (function () {
    ...
    var DEFAULT_OPTIONS = {
      ...
      numberOfDecimals: 2,
      cartItems: null,
    ClickOnRemoveFromCart:function(products, totalPrice, totalQuantity) {
        remove();
    } ,  
    ...
    */
    ...
    var setProduct = function (id, name, summary, price, quantity, image) {
      if (typeof id === "undefined") {
        console.error("رقم المنتج مطلوب");
        return false;
      }
      ...
      if (typeof image === "undefined") {
        console.error("الصورة مطلوبه");
        return false;
      }
      if (!$.isNumeric(price)) {
        console.error("السعر ليس رقم");
        return false;
      }
      if (!$.isNumeric(quantity)) {
        console.error("الكمية ليست رقم");
        return false;
      }
  ...
  var loadMyCartEvent = function (targetSelector) {
    ...
    if (!$("#" + idCartModal).length) {
      $('body').append(
        ...
        '<h4 class="modal-title" id="myModalLabel"><span class="glyphicon glyphicon-shopping-cart"></span> سلتي</h4>' +
        '</div>' +
        '<div class="modal-body" id="myDIV">' +
        '<table class="table table-hover table-responsive" id="' + idCartTable + '"></table>' +
           ' <thead><tr><th><input class="form-control" id="myInput" type="text" placeholder="Search.."></th></tr></thead>'+
        '</div>' +
        '<div class="modal-footer">' +
        '<button type="button" class="btn btn-default" data-dismiss="modal">إغلاق</button>' +
        '<button type="button" class="btn btn-primary ' + classCheckoutCart + '">الحساب</button>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>'
    ...
    var drawTable = function () {
      ...
      $cartTable.append(products.length ?
        ...
        '<td></td>' +
        '</tr>' :
        '<div class="alert alert-danger" role="alert" id="' + idEmptyCartMessage + '">سلتك فارغة</div>'
      );
$(document).ready(function(){
  $("#myInput").on("keyup", function() {
    var value = $(this).val().toLowerCase();
    $("#"+idCartTable+" tr").filter(function() {
